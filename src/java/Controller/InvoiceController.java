...
public class InvoiceController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String emailCashier = account != null ? account.getEmail() : null;
        ...
        if (OrderID != null) {          
            try {
                ...
                BigDecimal total = new BigDecimal(realValue);
                orderDAO.updateTotalPrice(total, OrderID);
                orderDAO.updateOrderStatus(OrderID, 1); 
                orderDAO.addEmailCashier(emailCashier, OrderID);
                response.sendRedirect("ShowT");
            } catch (SQLException e) {
                e.printStackTrace();
                System.out.println("loi catch");
