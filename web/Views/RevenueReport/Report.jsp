<%-- 
    Document   : newjsp
    Created on : Jul 17, 2024, 8:45:04 PM
    Author     : Thanh Cong
--%>
    ...
    <body>
        <div id="body">
            ...
            <div class="container">
                ...
                <p><fmt:formatDate value="${requestScope.dayR}" pattern="dd-MM-yyyy"/></p>
                <p>${requestScope.orderQuantity}</p>
                <p>${requestScope.dayRTo}</p>

                <form id="filterForm" action="${pageContext.request.contextPath}/revenueReport" method="get" class="form-inline">
                    <div class="form-group">
                        <label for="startDate">Start Date:</label>
                        <input type="text" id="startDate" name="startDate" class="form-control readonly-input" 
                               value="<fmt:formatDate value='${startDate}' pattern='dd-MM-yyyy'/>" readonly>
                    </div>
                    <div class="form-group">
                        <label for="endDate">End Date:</label>
                        <input type="text" id="endDate" name="endDate" class="form-control readonly-input" 
                               value="<fmt:formatDate value='${endDate}' pattern='dd-MM-yyyy'/>" readonly>
                    </div>
                    <div class="form-group">
                        <label for="filter">By:</label>
                        <select id="filter" name="filter" class="form-control" onchange="updateFilterOptions()">
                            <option value="Month" <c:if test="${filter eq 'Month'}">selected</c:if>>Month</option>
                            <option value="Quarter" <c:if test="${filter eq 'Quarter'}">selected</c:if>>Quarter</option>
                            <option value="Year" <c:if test="${filter eq 'Year'}">selected</c:if>>Year</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="filterValue">Value:</label>
                            <select id="filterValue" name="filterValue" class="form-control" onchange="submitForm()" required>
                            <c:choose>
                                <c:when test="${filter eq 'Month'}">
                                    <c:forEach var="i" begin="1" end="12">
                                        <option value="${i}" <c:if test="${filterValue eq i}">selected</c:if>>Month ${i}</option>
                                    </c:forEach>
                                </c:when>
                                <c:when test="${filter eq 'Quarter'}">
                                    <c:forEach var="i" begin="1" end="4">
                                        <option value="${i}" <c:if test="${filterValue eq i}">selected</c:if>>Quarter ${i}</option>
                                    </c:forEach>
                                </c:when>
                                <c:when test="${filter eq 'Year'}">
                                    <c:forEach var="year" items="${years}">
                                        <option value="${year}" <c:if test="${filterValue eq year}">selected</c:if>>${year}</option>
                                    </c:forEach>
                                </c:when>
                            </c:choose>
                        </select>
        ...
        <script>
            ...
            function updateFilterOptions() {
                ...
                while (filterValue.options.length) {
